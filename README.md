...
而“饮食日记”主要是一款可以识别果蔬、菜品的产品，主要面向所有年轻群体。它利用图像识别api中的果蔬识别和菜品识别，帮助用户识别果蔬的名称、菜品的名称、卡路里和其他信息，它既可以记录和分享自己的饮食生活，也可以与其他用户进行交流。另外，饮食和情感也是密不可分的，饮食可以影响情感，情感也会反过来影响饮食，运用自然语言处理api-情感倾向分析，分析饮食日记中编写的文案，分析用户的情感倾向，给日记形成一个情感标签，根据分析出的数值，连续记录一个星期形成情感曲线。
